...
qboolean Pickup_Fists (edict_t *ent, edict_t *other)
{
	int			index;
	gitem_t		*ammo;
...
void NoAmmoWeaponChange (edict_t *ent)
{
	...
	if ( ent->client->pers.inventory[ITEM_INDEX(FindItem("cells"))]
		&&  ent->client->pers.inventory[ITEM_INDEX(FindItem("Hands"))] )
	{
...
void Fire_Smaggd(edict_t *ent)
{
  ...
  fire_smaggd (ent, start, forward, 45, damage, 200, 1, MOD_PUNCH);

  ent->client->ps.gunframe++; 
  PlayerNoise(ent, start, PNOISE_WEAPON); 
}

void Weapon_Smaggd (edict_t *ent)
{
  static int  pause_frames[]  = {23, 45, 0};
  static int  fire_frames[] = {4, 0};

  Weapon_Generic (ent, 3, 5, 45, 49, pause_frames, fire_frames, Fire_Smaggd);
}
